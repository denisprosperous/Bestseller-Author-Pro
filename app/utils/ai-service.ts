...
export class AIService {
  ...
  private async getBestAvailableModel(provider: string, apiKey: string): Promise<string> {
    ...
    const latestModels: Record<string, string> = {
      openai: "gpt-3.5-turbo", // Use reliable model
      anthropic: "claude-3-5-sonnet-20241022", // Latest stable Claude
      google: "gemini-1.5-pro", // Latest stable Gemini
      xai: "grok-2-latest", // Latest stable Grok
      deepseek: "deepseek-llm-7b-instruct",
    };
    return latestModels[provider] || "gpt-3.5-turbo";
  }
  ...
  private async callDeepSeek(params: {
    ...
    // Use the router endpoint as recommended by Hugging Face
    // Fallback to deepseek-llm-7b-chat if model is just 'deepseek-llm-7b-instruct' which might be deprecated
    const modelId = params.model === 'deepseek-llm-7b-instruct' ? 'deepseek-ai/deepseek-llm-7b-chat' : params.model;
