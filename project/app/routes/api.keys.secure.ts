...
export async function action({ request }: ActionFunctionArgs): Promise<Response> {
  try {
    ...
    switch (action) {
      case 'save': {
        if (!provider || !apiKey) {
          return Response.json({ success: false, error: 'Provider and API key are required' }, { status: 400 });
        }
      ...
      case 'list': {
        ...
        return Response.json({ success: true, data: { providers: data || [] } });
      }
    ...
    return Response.json({ success: false, error: error instanceof Error ? error.message : 'API key operation failed' }, { status: 500 });
  }
}
...
export async function loader({ request }: LoaderFunctionArgs): Promise<Response> {
  try {
    ...
    return Response.json({ success: false, error: error instanceof Error ? error.message : 'Failed to load API keys' }, { status: 500 });
  }
}
