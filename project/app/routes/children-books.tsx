...
interface ChildrensBookData {
  ...
  pages: BookPage[];
  illustrationStyle: NonNullable<ImageGenerationRequest['style']>;
}
    ...
    switch (action) {
      case 'generate-story': {
        ...
        const bookData: ChildrensBookData = {
          ...
          illustrationStyle: illustrationStyle as NonNullable<ImageGenerationRequest['style']>
        };
      ...
      case 'generate-illustrations': {
          ...
          bookData.characters.map(async (character) => {
            try {
              const imageResult = await imageGenerationService.generateImage({
                ...
                provider: 'google-vertex', // Use Google Vertex AI
                aspectRatio: '1:1'
              });
        ...
        const updatedPages = await Promise.all(
          bookData.pages.map(async (page) => {
            try {
              const imageResult = await imageGenerationService.generateImage({
                prompt: `${updatedCharacters[0].visualPrompt}, ${page.illustrationPrompt}`,
                style: bookData.illustrationStyle,
              ...
              return {
                ...page,
                illustrationUrl: imageResult.imageUrl
              };
            } catch (error) {
